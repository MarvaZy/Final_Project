...
class Search(db):
    ...
    def _check_if_valid(self, list_of_words):
        '''
    ...
    def search(self):
        ...
        # checks for possible type errors in user input for restrictions
        restrictions = input("מהן המגבלות התזונתיות? (ניתן לבחור יותר מאחד, יש להקיש רווח בין בחירה לבחירה. אנא כתבו רכיבים בצורת יחיד)\n").split()
        while not self._check_if_valid(restrictions):
            restrictions = input("לשיקולך, הזן כעת את בחירותייך בשנית. אם אינך מעוניין לשנות את בחירותייך, הקש אנטר\n").split()
            if len(restrictions) == 0:
                break
        restrictions = self._singular_to_ploral(restrictions)
        
        # checks for possible type errors in user input for preferences
        preferences = input("אילו מצרכים תרצה שיכללו במתכון? (ניתן לבחור יותר מאחד, יש להקיש רווח בין בחירה לבחירה. אנא כתבו רכיבים בצורת יחיד)\n").split()
        while not self._check_if_valid(preferences):
            preferences = input("לשיקולך, הזן כעת את בחירותייך בשנית. אם אינך מעוניין לשנות את בחירותייך, הקש אנטר\n").split()
            if len(preferences) == 0:
                break
        preferences = self._singular_to_ploral(preferences)
